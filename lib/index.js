...
//
    ...
    */
    ...
    Workbook.prototype.extractPlaceholders = function(string) {
        ...
        const replaceHashWithTable = string.replace(new RegExp('#', 'g'), 'table:');
        
        var match = null, matches = [];
        while((match = re.exec(replaceHashWithTable)) !== null) {
            matches.push({
                placeholder: match[0],
                type: match[1] || 'normal',
                name: match[2],
                key: match[3],
                subType: match[4],
                full: string.includes("#")|| match[0].length === string.length
            });
        }
        return matches;
    };
